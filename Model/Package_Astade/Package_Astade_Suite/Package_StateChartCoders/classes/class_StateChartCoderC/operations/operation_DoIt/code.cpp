//~~ int DoIt(int argc, char* const* argv) [StateChartCoderC] ~~
...
CmdLineParser.SetLogo(wxS("StateChartCoderC: the \"C state-chart coder\"\n" COPYRIGHT));
...
myImplname.SetExt(wxS("c"));
impl.open(myImplname.GetFullPath().char_str());
aPrologue.SetExt(wxS("c"));
