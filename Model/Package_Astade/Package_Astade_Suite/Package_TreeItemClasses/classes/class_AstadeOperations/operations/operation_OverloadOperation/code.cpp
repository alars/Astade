...
else
{
    ...
    if (pos2 != wxNOT_FOUND && pos2 > pos + 1)
    {
        ...
        if (pos3 == wxNOT_FOUND)
        {
            ...
            wxFileName parms = AdeParameters::CreateNewElement(newFile);
            wxFileName param = AdeParameter::CreateNewElement(parms, wxS("port"),
                                signature(pos + 1, pos3 - pos - 1));
            AdeParameter* tmpParameter = new AdeParameter(param);
            tmpParameter->SetDescription(wxS("the port where this event comes from"));
            tmpParameter->SetIsInput(true);
            delete tmpParameter;

            param = AdeParameter::CreateNewElement(parms, wxS("event"),
                                signature(pos3 + 2, pos2 - pos3 - 2));
            tmpParameter = new AdeParameter(param);
            tmpParameter->SetDescription(wxS("the event which caused this call"));
            tmpParameter->SetIsInput(true);
            delete tmpParameter;
        }
